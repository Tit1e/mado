 ...
 * [OUTPUT]: 验证 preload 与主进程 IPC 频道一致，并校验项目目录选择、Codex/Pi 启动与命令重启链路
...
test('应用菜单通过同一受控桥接启动 Codex 与 Pi', async () => {
  const root = path.resolve(__dirname, '..', '..');
  const main = await fsp.readFile(path.join(root, 'electron', 'main.js'), 'utf8');
  const preload = await fsp.readFile(path.join(root, 'electron', 'preload.js'), 'utf8');
  assert.match(main, /accelerator:\s*'CmdOrCtrl\+Shift\+N'/);
  for (const payload of [
    "{ agent: 'codex', action: 'preferred' }", "{ agent: 'codex', action: 'new' }",
    "{ agent: 'pi', action: 'preferred' }", "{ agent: 'pi', action: 'new' }",
  ]) assert.ok(main.includes(`send('terminal:launch-agent', ${payload})`));
  assert.match(preload, /onLaunchAgent:[\s\S]*ipcRenderer\.on\('terminal:launch-agent'/);
});
