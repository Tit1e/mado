 ...
 * [OUTPUT]: 对外提供 MADO_DICT 静态词典和 MADO_DICT_RULES 动态翻译规则，覆盖手动项目、Codex/Pi 启动与项目运行命令
...
window.MADO_DICT = {
  ...
  // ---------- 欢迎页（首次引导，按文本片段拆分）----------
  '使用指南': 'User guide',
  '欢迎用 Mado': 'Welcome to Mado',
  '从找项目、运行 Agent 到核对改动，常用能力都在这一个窗口。': 'Find projects, run an Agent, and review its changes without leaving this window.',
  '核心工作流': 'Core workflow',
  ...
  '启动 Agent': 'Launch Agents',
  '终端工具栏可一键启动 Codex 或 Pi，终端设置可统一选择继续最近会话或新建会话；把文件或文件夹拖进终端即可加入上下文。': 'Launch Codex or Pi from the terminal toolbar. Terminal settings choose whether quick launch continues the latest session or starts a new one. Drag files or folders into the terminal to add context.',
  '跟踪与核对改动': 'Track and review changes',
  '开启文件跟随后，文件区和预览会追踪当前终端中的 Agent；Git 状态可直接打开工作区 Diff。': 'File Follow keeps the file view and preview on the Agent in the current terminal. Open the working-tree diff directly from Git status.',
  ...
  '启动 Pi': 'Launch Pi',
  '在左侧当前目录继续最近会话': 'Continue the latest session in the current folder on the left',
  '在左侧当前目录启动新会话': 'Start a new session in the current folder on the left',
  '终端设置': 'Terminal settings',
  '一键启动时继续左侧当前目录最近的会话': 'Continue the latest session in the current folder on the left when using quick launch',
  '继续最近会话': 'Continue latest session',
  'Agent 等待确认或任务完成时播放提示音': 'Play a chime when an Agent needs approval or finishes a task',
  'Agent 提示音': 'Agent chime',
  '一键启动将继续最近会话': 'Quick launch will continue the latest session',
  '一键启动将创建新会话': 'Quick launch will create a new session',
  'Agent 提示音已关闭': 'Agent chime disabled',
  'Agent 提示音已开启': 'Agent chime enabled',
  '正在继续最近会话': 'Continuing the latest session',
  '正在启动新会话': 'Starting a new session',
  ...
  '文件跟随：跟着当前终端 tab 的 Agent，它改哪个文件，文件区和预览就跟到哪；html/md 边写边出实时预览（手动浏览即自动停）': "Follow files: tracks the Agent in the current terminal tab — the file view and preview follow whatever it edits; html/md render live as they are written (any manual browsing stops it)",
  '文件跟随': 'Follow files',
  ...
  '不支持的 Agent 启动方式': 'Unsupported Agent launch action',
  '先点开一个终端 tab，跟随才知道盯哪个 Agent 会话': 'Open a terminal tab first so Follow knows which Agent session to track',
  '等待 Agent 写文件…': 'Waiting for the Agent to write a file…',
...
window.MADO_DICT_RULES = [
  ...
  // 终端
  [/^Agent 刚生成(.*)$/, (m) => `Just generated by the Agent${m[1]}`],
  [/^Agent 正在 (.*)$/, (m) => `Agent is ${m[1]}`],
