<!--
[INPUT]: 依赖 public/vendor 的本地依赖版本和上游 xterm 修复状态
[OUTPUT]: 对外提供 vendor 补丁历史、守卫规则和退场条件
[POS]: docs 的第三方依赖补丁档案，防止升级时重复引入或遗漏修复
...
- **改动**（minified 单行内查找替换，另在文件头加了 `MADO-PATCH` 注释）：
