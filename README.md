...
- **手动项目列表 / Manual projects** — 在桌面版侧栏通过系统目录选择器添加项目，也可右键文件区空白处或文件夹直接加入；列表持久保存、自动去重、按名称自然排序且不依赖任何 Agent 会话。<br>
  Add projects with the desktop folder picker, or right-click file-area whitespace or a folder. The persistent, deduplicated list uses natural name sorting and stays independent of Agent session history.
