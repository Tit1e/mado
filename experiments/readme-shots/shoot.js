 ...
 * [OUTPUT]: 对外提供 README 使用的三套 Mado 实拍截图
...
const FAKE_HOME = process.env.MADO_SCREENSHOT_HOME || '/tmp/mado-readme-shots';
process.env.HOME = FAKE_HOME;
process.env.MADO_DEV_PORT ||= '4621';
...
function prepareHome() {
  ...
  fs.writeFileSync(path.join(FAKE_HOME, 'README.md'), '# Mado\n\n本地文件驾驶舱，列表浏览、原地预览、内嵌终端。\n');
  fs.writeFileSync(path.join(FAKE_HOME, 'notes.md'), '# 今日记录\n\nCodex 正在整理当前项目。\n');
  fs.writeFileSync(path.join(FAKE_HOME, 'package.json'), JSON.stringify({ name: 'mado-sample', private: true }, null, 2));
}
...
async function waitForWindow() {
  ...
  throw new Error('Mado window did not become ready');
}
...
app.whenReady().then(async () => {
  try {
    ...
    await win.webContents.executeJavaScript(`
      localStorage.setItem('mado_guided', '1');
      localStorage.setItem('mado_view', 'grid');
      localStorage.setItem('mado_gridsize', 'lg');
